...
sub toPrint {
	...
	$txt .= $self->getGeneName()."\t".$self->getNM()."\t".$self->getENST()."\t".$self->getENSP()."\t".$self->getHGNC()."\t".$self->getUniprot()."\t".$self->getNMVersion()."\t".$self->getNG()."\t".$self->getNP()."\t".$self->getMain()."\t".$self->getChr()."\t".$self->getStrand()."\t".$self->getNbExons()."\t".$self->getTss()."\t".$self->getProtName()."\t".$self->getShortProt()."\t".$self->getProtSize()."\n";
	return $txt;
}

sub toSQL {
	my $self = shift;
	my $sql = "INSERT INTO gene (nom, second_name, chr, brin, nbre_exons, nom_prot, short_prot, taille_prot, uniprot_id, acc_version, gi_NM, acc_g, gi_NG, acc_p, gi_NP, ENST, ENSP,  translation_start_site, mutalyzer_version, main, USHER, RP, DFN, EXTENDED_DFN, \"MiSeq-28\", \"MiSeq-112\", \"MiSeq-121\", \"MiSeq-132\", \"MiSeq-3\", \"MiniSeq-3\", \"MiniSeq-121\", \"MiniSeq-132\", \"NextSeq-ClinicalExome\") VALUES ( '{\"".$self->getGeneName()."\",\"".$self->getNM()."\"}','".$self->getSecondName()."','".$self->getChr()."','".$self->getStrand()."','".$self->getNbExons()."','".$self->getProtName()."','".$self->getShortProt()."','".$self->getProtSize()."','".$self->getUniprot()."','".$self->getNMVersion()."','NULL','".$self->getNG()."','NULL','".$self->getNP()."','NULL','".$self->getENST()."','".$self->getENSP()."','".$self->getTss()."','".$self->getMutalyzerVer()."','".$self->getMain()."','".$self->getUsh()."','".$self->getRP()."','".$self->getDfn()."','".$self->getExtDfn()."','".$self->getMS28()."','".$self->getMS112()."','".$self->getMS121()."','".$self->getMS132()."','".$self->getMS3()."','".$self->getMnS3()."','".$self->getMnS121()."','".$self->getMnS132()."','".$self->getNxTSqCE()."');\n";
	return $sql;
}
#".$self->get().",
